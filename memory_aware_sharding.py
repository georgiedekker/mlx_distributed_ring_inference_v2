...
class ShardingPlan:
    ...
    def __post_init__(self):
        self.memory_utilization = (self.estimated_memory_mb / self.available_memory_mb 
                                 if self.available_memory_mb > 0 else float('inf'))
...
def estimate_layer_memory(layer: nn.Module, config, batch_size: int = 1, 
                         seq_len: int = 2048, dtype_size: int = 2) -> LayerEstimate:
    ...
    active_expert_activations = (
        batch_size * seq_len * experts_per_token * moe_intermediate
    )
...
def estimate_model_memory(model, config, batch_size: int = 1, 
                         seq_len: int = 2048) -> List[LayerEstimate]:
    ...
    for i, layer in enumerate(layers):
        ...
        logger.debug(f"Layer {i}: {estimate.total_mb:.1f} MB "
...
def create_memory_aware_sharding_plan(
    ...
    for info in sorted_ranks:
        ...
        logger.info(f"Rank {info.rank}: {effective_mb:.1f} MB effective memory "
    ...
    for layer_idx, layer_est in sorted_layers:
        ...
        if available >= layer_est.total_mb:
            ...
            logger.debug(f"Assigned layer {layer_idx} ({layer_est.total_mb:.1f} MB) to rank {target_rank}")
        else:
            ...
            if best_rank is not None:
                ...
                logger.debug(f"Assigned layer {layer_idx} ({layer_est.total_mb:.1f} MB) to rank {best_rank} (fallback)")
            else:
                # No rank can fit this layer - issue warning and assign to highest capacity rank
                logger.warning(f"Layer {layer_idx} ({layer_est.total_mb:.1f} MB) cannot fit on any rank!")
        ...
        layers_target_for_current = layers_per_rank_target + (1 if current_rank_idx < extra_layers else 0)
        
        if layers_assigned_to_current >= layers_target_for_current and current_rank_idx < len(sorted_ranks) - 1:
            current_rank_idx += 1
    ...
    for info in memory_infos:
        ...
        if assigned_layers:
            ...
            if num_layers_assigned != (end_layer - start_layer + 1):
                logger.warning(f"Rank {rank} has non-contiguous layer assignment: {assigned_layers}")
        ...
        logger.info(f"Rank {rank}: layers {start_layer}-{end_layer} ({num_layers_assigned} layers), "
    ...
    if total_assigned != num_layers:
        logger.warning(f"Layer assignment mismatch: {total_assigned} assigned vs {num_layers} total")
    
    return sharding_plans

def validate_sharding_plan(sharding_plans: List[ShardingPlan], 
    ...
    for plan in sharding_plans:
        if plan.memory_utilization > 1.0:
            issues.append(f"Rank {plan.rank} memory overcommitted: "
                         f"{plan.memory_utilization:.1%}")
...
def log_sharding_summary(sharding_plans: List[ShardingPlan], 
                        layer_estimates: List[LayerEstimate]):
    ...
    for plan in sorted(sharding_plans, key=lambda x: x.rank):
        if plan.num_layers > 0:
            utilization_color = "HIGH" if plan.memory_utilization > 0.9 else "MEDIUM" if plan.memory_utilization > 0.7 else "LOW"
            logger.info(f"Rank {plan.rank:2d}: Layers {plan.start_layer:2d}-{plan.end_layer:2d} "
    ...
    logger.info(f"Total memory utilization: {memory_used:.1f} MB / {memory_available:.1f} MB ({avg_utilization:.1%})")
    ...
    logger.info(f"Utilization range: {min_utilization:.1%} - {max_utilization:.1%} (spread: {utilization_range:.1%})")
