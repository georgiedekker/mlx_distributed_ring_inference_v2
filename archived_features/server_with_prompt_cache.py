    ...
    def get_or_create_cache(self, conversation_id: str, prompt: str, formatted_prompt: str):
        ...
        if conversation_id in self.cache_store:
            ...
            if common_prefix_len > 0:
                ...
                logger.info(
                    f"Cache partial hit: reusing {common_prefix_len}/{len(current_tokens)} tokens ({reuse_ratio:.1%})"
                )
        ...
        if has_work.item() > 0:
            ...
            if rank == 0:
                ...
                if cache_hit and reused_tokens > 0:
                    ...
                    speedup_factor = (
                        total_tokens / (total_tokens - reused_tokens)
                        if total_tokens > reused_tokens
                        else 1.0
                    )

                logger.info(
                    f"Generated response for conversation {conversation_id}: '{response_text}'"
                )
                logger.info(
                    f"Cache status: {'HIT' if cache_hit else 'MISS'} | Reused: {reused_tokens} tokens | Speedup: {speedup_factor:.1f}x"
                )
                logger.info(
                    f"Timing - Prompt: {prompt_time:.2f}s, Generation: {generation_time:.2f}s, Total: {total_time:.2f}s"
                )
